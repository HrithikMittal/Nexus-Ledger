    ...
    dbo
      ...
      .toArray(function(err, result) {
        ...
        for (i = 0; i < result.length; i++) {
          if (result[i].nameledger == value2) {
            for (k = 0; k < result[i].getfromledger.getmoney.length; k++) {
              valueget =
                valueget + parseInt(result[i].getfromledger.getmoney[k]);
            }
            ...
            newcredit = parseInt(result[i].creditamount);
        ...
        if (flagagain == 0) {
          ...
          newdebitz = parseInt(value5);
          ledgera.debitamount = newdebitz;
          newcreditz = 0;
          ledgera.creditamount = newcreditz;
          ...
          dbo.collection("ledger").updateOne(myorga, {
            $set: {
              debitamount: setamountintial,
              creditamount: setamountc
            }
          });
        }
      });
